...
class User:
    def __init__(self, data):
        ...
        self.attending = User.getEventsAttending({'id':data['id']})
...
#get events users attending
    @classmethod
    def getEventsAttending(cls, data):
        query = 'select * from events left join attendees on attendees.user_id where user_id = %(user_id)s'
        results = connectToMySQL(DATABASE).query_db(query, data)
        print(results)
        attending = []
        if results:
