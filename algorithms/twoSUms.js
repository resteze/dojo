...
function twoSumEffecient(nums, targetSum) {
    const hashmap = {}
    for (let i = 0;i<nums.length;i++){
        const val = nums[i]
        const otherHalf = targetSum - val
        if(hashmap.hasOwnProperty(val.toString())){
// is more effecient because it searches an object specifically versus going through the whole array
            return [map[otherHalf], i]
        }else{
            map[otherHalf] = i
// if not in the dictionary adds the otherHalf into it for that index
        }
    }
}
